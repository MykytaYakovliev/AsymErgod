...
columns <- c('n_trajectories', 'h', 'sigma', 'kappa', 'H',
             'estimator', 'measure',
             paste0('h', c(1,10,50,100,250,500,1000)))
  ...
  for (kappa in c(2.5)) {
    for (H in c(0.1,0.2,0.3,0.4,0.6,0.7)) {
      ...
      for (ii in 1:n_iter) {
        ...
        vecest <- c()
        for (hj in 1/c(1,10,50,100,250,500,1000)) {
          Xkn <- generate.Xkn(n, hj, H, sigma, kappa) # generate process X
          vecest <- rbind(vecest, estimators(Xkn, n, n, hj,
                                             H, sigma, kappa))
        }
      ...
      file <- paste("Z:\\University\\Ральченко\\2022-06-10 Асимптотична нормальність\\Modeling\\Update\\results_h", name, sep="")
